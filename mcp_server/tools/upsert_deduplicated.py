"""Upsert chunks with deduplication - keep only first occurrence of each ID."""
...
class DeduplicatedUpserter:
    """Upsert unique chunks only."""
    ...
    def upsert_all(self):
        """Load chunks, deduplicate, and upsert."""
        logger.info("Loading all chunks...")
        chunks_dict = {}  # Use dict to automatically deduplicate by ID
        ...
        for chunk_file in sorted(data_dir.glob('skill_chunks_*.json')):
            ...
            with open(chunk_file) as f:
                ...
                for chunk in data.get('chunks', []):
                    ...
                    if cid and cid not in chunks_dict:  # Keep first occurrence only
                        chunks_dict[cid] = chunk

        chunks = list(chunks_dict.values())
        logger.info(f"Loaded {len(chunks)} unique chunks (deduplicated from 2826)\n")
        ...
        for i in range(0, len(chunks), batch_size):
            ...
            for chunk in batch:
                ...
                # TRIMMED metadata
                metadata = {
                    'skill_name': chunk.get('skill_name', '')[:100],
                    'chunk_type': chunk.get('chunk_type', ''),
                    'api_names': json.dumps(chunk.get('api_names', [])[:10]),
        ...
        logger.info(f"  Upserted: {upserted} unique chunks")
        logger.info(f"  Failed: {failed}")
        logger.info(f"{'='*50}")


if __name__ == "__main__":
    upserter = DeduplicatedUpserter()
    upserter.upsert_all()
